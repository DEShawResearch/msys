...
def remove_periodic_contacts(_mol, npro, dist):
    ...
    mol.append(wat)
    sel = (
        "index < %(natoms)d and index >= %(npro)d and within %(dist)f of (index >="
        " %(natoms)d)"
        % locals()
    )
    pos = mol.positions
...
def solvate(
    ...
    if center_selection != "none":
        ...
        if not ids:
            raise ValueError(
                "Center selection '%s' selected no atoms" % center_selection
            )
        pos = mol.positions
        pos -= pos[ids].mean(0)
        mol.positions = pos
    ...
    if verbose:
        print("After removing overlap, %d solvent atoms" % (mol.natoms - npro))

    # remove molecules whose center is outside the desired box
    xmin, ymin, zmin = dmin
    xmax, ymax, zmax = dmax
    hits = mol.select(
        "index >= %d and (x<%s or y<%s or z<%s or x>%s or y>%s or z>%s)"
        % (npro, xmin, ymin, zmin, xmax, ymax, zmax)
    )
    frags = set(a.fragid for a in hits)
    if frags:
        fragmap = dict()
        sel = "(%s) and fragid %s" % (solvent_selection, " ".join(map(str, frags)))
        for a in mol.select(sel):
            fragmap.setdefault(a.fragid, []).append(a.id)
        outside = []
        ...
        for fragid, ids in fragmap.items():
            c = pos[ids].mean(0)
            if (c < pmin).any() or (c > pmax).any():
                outside.append(fragid)
        if outside:
            mol = mol.clone("not fragid " + " ".join(map(str, outside)))
            if verbose:
                print(
                    "After removing outside solvent molecules, %d solvent atoms"
                    % (mol.natoms - npro)
                )
    ...
    if verbose:
        print("after removing periodic clashes, %d solvent atoms" % (mol.natoms - npro))
...
def parser():
    ...
    parser.add_argument("solute", help="Path to chemical system to be solvated")
    parser.add_argument(
        "-s",
        "--solvent",
        default=os.path.realpath(default_solvent),
        help="Path to chemical system containing solvent molecules",
    )
    parser.add_argument(
        "-o", "--output", default="solvate.dms", help="Path to output file"
    )
    parser.add_argument(
        "-c",
        "--center-selection",
    ...
    grp.add_argument(
        "-b", "--box", help="water box dimensions: 1 or 3 comma-separated values"
    )
    grp.add_argument(
        "-t",
        "--thickness",
