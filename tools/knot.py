...
def FindKnots(
    ...
    if not cycles:
        return []
    if verbose:
        print("Largest cycle has size %d" % max(list(map(len, cycles))))
    ...
    if max_cycle_size is not None:
        ...
        if verbose:
            print(
                "Reduced to %d cycles of length <= %d" % (len(cycles), max_cycle_size)
            )
    ...
    try:
        ...
        if ignore_excluded_knots:
            raise ValueError(
                "Cannot ignore_excluded_knots without an exclusion table present."
            )

    def is_excluded_knot(cycle, bond):
        for cycle_atomid in cycle:
            for bond_atomid in bond:
                if not exclusion_table.findWithAll(
                    [mol.atom(cycle_atomid), mol.atom(bond_atomid)]
                ):
                    return False
        return True
                    ...
                    for idx in cycle_inds:
                        ...
                        if LineIntersectsTriangle(
                            ...
                            if ignore_excluded_knots and is_excluded_knot(cycle, bond):
                                continue
                            if verbose:
                                print("==> intersection:", cycle, bond)
                            results.append((cycle, bond, idx))
                            found.add(key)
    if verbose:
        print("Total intersections: %d" % len(found))

    t4 = time()

    if False:
        print(
            "Time: %8.3f %8.3f %8.3f %8.3f"
...
def get_lipid_atoms(cycle, bond, dms):
    cycle_ids = [
        atom.id for atom in dms.select("lipid and same residue as index %d" % cycle[0])
    ]
    bond_ids = [
        atom.id for atom in dms.select("lipid and same residue as index %d" % bond[0])
    ]
    assert (
        cycle_ids or bond_ids
    ), "Neither the cycle %s or the bond %s was part of a lipid" % (cycle_ids, bond_ids)
    if cycle_ids and bond_ids:
        print(
            "WARNING: Both the cycle %s and the bond %s were part of a lipid. Will only"
            " move the bond molecule."
            % (cycle_ids, bond_ids)
        )
...
def fix_thread(mol, cycle, bond, idx, move_lipid, dirty, verbose=True):
    ...
    if set(atomsel) & dirty:
        if verbose:
            print(
                "Will not fix %s %s this iteration because atoms that must be moved"
                " have been moved earlier this pass."
                % (cycle, bond)
            )
        return []
...
def UntieKnots(
    ...
    for iteration_number in range(iteration_limit):
        ...
        intersections = FindKnots(
            mol,
            max_cycle_size=max_cycle_size,
            selection=selection,
            ignore_excluded_knots=ignore_excluded_knots,
            verbose=verbose,
        )
        if not intersections:
            return True
...
def main():
    ...
    parser = OptionParser(
        "Usage: dms_find_knot [options] system.dms.\n\nGiven a dms file, find any"
        " instances where a bond is 'threaded' through a ring."
    )
    ...
    parser.add_option(
        ...
        default=10,
        help=(
            "Maximum cycle size to check (if this is set too large,"
            " disulfide-bond-induced rings will be checked)."
        ),
    )
    ...
    parser.add_option(
        "--untie",
        action="store_true",
        help="Attempt to remove any knots found and write to --outfile",
        default=False,
    )
    parser.add_option(
        "--outfile",
        "-o",
        type=str,
        help="Output dms file; for use in combination with --untie",
        default=None,
    )
    parser.add_option(
        "--move-lipid",
        action="store_true",
        help=(
            "Use with --untie; move the whole lipid molecule instead of just the two"
            " offending bond atoms"
        ),
        default=False,
    )
    parser.add_option(
        "--ignore-excluded-knots",
        action="store_true",
        default=False,
        help=(
            "Ignore knots where the bond atoms are not interacting with the ring atoms,"
        )
