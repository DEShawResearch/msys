...
class HMRActions(argparse.Action):
    def __call__(self, parser, namespace, values, option_string):
        if len(values) != 2:
            raise RuntimeError(f"I need 2 values, got: {values}")
        sel = values[0]
        mass = None
        try:
            mass = float(values[1])
        ...
        if mass is None:
            raise RuntimeError(
                f"Second argument to {option_string} must be the mass, got: {values[1]}"
            )
        repartition = {
            "--repartition": True,
            "-r": True,
...
"""
    ...
    parser.add_argument(
        ...
        dest="actions",
        nargs=2,
        metavar=("SELECTION", "MASS"),
        help=(
            "Set mass of hydrogens in SELECTION to MASS but do NOT repartition from the"
            " bonded heavy atom"
        ),
    )
    parser.add_argument(
        ...
        metavar=("SELECTION", "MASS"),
        help=(
            "Set mass of hydrogens in SELECTION to MASS by repartitioning from the"
            " bonded heavy atom"
        ),
    )
...
def adjust_hmasses(mol, sel, hmass, repartition):
    ...
    if warn:
        s = (
            "  * WARNING: It looks like the hydrogen or bonded atom masses in"
            f' selection "{sel}" were already modified!'
        )
        s += (
            "\n    You may have overlapping selections or are modifing a file that"
            " already has hmr applied."
        )
    ...
    for a in hatoms:
        ...
        if repartition:
            ...
            assert (
                batom.mass > 0
            ), f"Mass for atom {batom.id} ({batom.name}) is < 0 ({batom.mass})"
