...
def main():
    ...
    parser.add_option(
        "-f", type="float", default=0.0, help="force constant in PEAK units"
    )
    parser.add_option(
        "-x", default=None, help="force constant along x axis in PEAK units"
    )
    parser.add_option(
        "-y", default=None, help="force constant along x axis in PEAK units"
    )
    parser.add_option(
        "-z", default=None, help="force constant along x axis in PEAK units"
    )
    parser.add_option(
        "-s", "--selection", default="none", help="selection for restrained atoms"
    )
    parser.add_option(
        "--replace",
        action="store_true",
    ...
    parser.add_option(
        "-q", "--quiet", action="store_true", default=False, help="Don't be chatty"
    )
    ...
    if not opts.quiet:
        print("Adding restraints to %d atoms" % len(atoms))
    ...
    if not opts.quiet:
        print("Using force constant (%s, %s, %s)" % (fcx, fcy, fcz))

    ref = opts.reference_structure
    if ref:
        if not opts.quiet:
            print("Loading reference file <%s>" % ref)
        ref_mol = msys.LoadDMS(ref)
        if not opts.quiet:
            print("Restraining atoms to reference positions")
