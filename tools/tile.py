...
def main():
    ...
    mols = args.inputs
    if n%len(mols)!=0:
        print("Number of input files (%d) is incommensurate with number of replicas (%d)" % (len(mols), n))
        return 1
    ...
    for i, mol in enumerate(mols):
        ...
        for celldim, extent in zip(numpy.diag(cell), posrange):
            if celldim * 1.5 < extent:
                print("Cannot tile system %d: box size appears to be much smaller than spatial extent of particles" % (i+1))
                return 1
