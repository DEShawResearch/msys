...
/* Modified by Justin Gullingsrud, D. E. Shaw Research
 * - obtained from VMD molfile plugins; public domain.
 * - converted to C++-legal C.
 * - convert box dimensions to full unit cell
 */
...
static void vtf_create_atoms_as_needed(int max_aid, vtf_data *d) {
  ...
  if (d->natoms < max_aid+1) {
    int aid;
      ...
      d->atoms = (molfile_atom_t*)realloc(d->atoms, (max_aid+1)*sizeof(molfile_atom_t));
...
static int *vtf_parse_aid_specifier(char *s, vtf_data *d) {
  ...
  if (s[0] == 'd') {
    ...
    while (1) {
      ...
      if (sscanf(s, " %u:%u%n", &from, &to, &n) == 2) {
        ...
        offset = size;
        size += to-from+1;
        aid_list = (int *)realloc(aid_list, size*sizeof(int));
  ...
  aid_list = (int *)realloc(aid_list, (size+1)*sizeof(int));
  aid_list[size] = -1;
  return aid_list;
}
  ...
  while (sscanf(s, " %255s%n", keyword, &n) == 1) {
    ...
    case 'r': {
      ...
      if (strlen(keyword) == 1 || 
        ...
        /* radius */
        if (sscanf(s, " %lf%n", &atom.radius, &n) == 1) {
    ...
    case 'q': {
      ...
      if (strlen(keyword) == 1 ||
        ...
        if (sscanf(s, " %lf%n", &atom.charge, &n) == 1) {
    ...
    case 'o': {
      /* occupancy */
      if (sscanf(s, " %lf%n", &atom.occupancy, &n) == 1) {
    ...
    case 'b': {
      /* bfactor */
      if (sscanf(s, " %lf%n", &atom.bfactor, &n) == 1) {
    ...
    case 'm': {
      /* mass */
      if (sscanf(s, " %lf%n", &atom.mass, &n) == 1) {
...
static int vtf_parse_bond(char *line, vtf_data *d) {
  ...
  while (1) {
    if (sscanf(s, " %u::%u%n", &from, &to, &n) == 2) {
      ...
      bid = d->nbonds;
      d->nbonds += to-from;
      d->from = (int *)realloc(d->from, d->nbonds*sizeof(int));
      d->to = (int *)realloc(d->to, d->nbonds*sizeof(int));
      /* TODO: error handling */
      for (aid = from; aid < to; aid++) {
...
_vtf_open_file_read(const char *filepath, 
  ...
  d = (vtf_data*)malloc(sizeof(vtf_data));
...
static int vtf_read_next_timestep(void *data, 
  ...
  if (d->coords == NULL) {
    /* initialize coords */
    d->coords = (float *)malloc(natoms*3*sizeof(float));
    /* TODO: error handling */
  ...
  if (ts != NULL) {
    ...
    void ImportPDBUnitCell(double A, double B, double C,
                           ...
                           double* cell_3x3);
    desres::msys::ImportPDBUnitCell(d->A, d->B, d->C, d->alpha, d->beta, d->gamma, ts->unit_cell);
    //ts->A = d->A;
    //ts->B = d->B;
    //ts->C = d->C;
    //ts->alpha = d->alpha;
...
extern "C"
int msys_vtfplugin_register(void *v, vmdplugin_register_cb cb) {
