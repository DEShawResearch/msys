...
static inline void swap4_unaligned(void *v, long ndata) {
  long i;
  char *dataptr;
  char tmp;
...
static inline void swap8_unaligned(void *v, long ndata) {
  char *data = (char *) v;
  long i;
  char byteArray[8];
  char *bytePointer;
...
static inline void swap2_aligned(void *v, long ndata) {
  short *data = (short *) v;
  long i;
  short *N; 
...
/* on some platforms if used on unaligned data.                      */
static void inline swap4_aligned(void *v, long ndata) {
  int *data = (int *) v;
  long i;
  int *N;
  for (i=0; i<ndata; i++) {
...
/* on some platforms if used on unaligned data.                      */
static inline void swap8_aligned(void *v, long ndata) {
