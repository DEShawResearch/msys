...
/* enable DESRES-specific features */
#ifndef DESRES_READ_TIMESTEP2
#define DESRES_READ_TIMESTEP2
#endif

#ifndef DESRES_CTNUMBER
#define DESRES_CTNUMBER
#endif
...
typedef struct {
  ...
  double occupancy;   /**< optional occupancy value              */
  double bfactor;     /**< optional B-factor value               */
  double mass;        /**< optional mass value                   */
  double charge;      /**< optional charge value                 */
  double radius;      /**< optional radius value                 */
...
#define MOLFILE_PARTIALTS     0x0400  /**< read/write partial frames       */
...
typedef struct molfile_timestep_metadata {
  molfile_ssize_t count;               /**< total # timesteps; -1 if unknown */
  molfile_ssize_t avg_bytes_per_timestep; /** bytes per timestep             */
  int has_velocities;                  /**< if timesteps have velocities     */
  int supports_double_precision;       /**< can use dcoords, dvelocities     */
...
typedef struct {
...
#if defined(DESRES_READ_TIMESTEP2)
  /* Unit cell as Ax, Ay, Az, Bx, By, Bz, Cx, Cy, Cz */
  double unit_cell[9];
#else
  /*@{*/   
  /**
...
#if defined(DESRES_READ_TIMESTEP2)
  /* support generic trajectory information */
  double total_energy;
  ...
  /* pressure tensor as Pxx, Pxy, Pxz, Pyx,... Pzy, Pzz */
  double pressure_tensor[9];  

  /* virial tensor as Vxx, Vxy, Vxz, Vyx, ... Vzy, Vzz */
  double virial_tensor[9];   
...
typedef struct {
  ...
  /**
   * Sync to disk.  MOLFILE_SUCCCESS on success, MOLFILE_ERROR on error.
   */
  int (* sync_file_write)(void *);
...
#endif
  int (* truncate_file_write)(void *, double t);
