...
static void *open_dcd_read(const char *path, const char *filetype, 
   ...
   */
  {
    ...
    dcd->setsread = 0;
    dcd->start = curpos;
    dcd->firstframesize = firstframesize;
    dcd->framesize = framesize;
  }
  ...
  if (dcd->nfixed && dcd->nsets>0) {
      /* just read the fixed block now so that we can do random access later.
       */
      int rc;
      float unitcell[6];
      int first = 1;
      rc = read_dcdstep(dcd->fd, dcd->natoms, dcd->x, dcd->y, dcd->z, 
          unitcell, dcd->nfixed, first, dcd->freeind, dcd->fixedcoords, 
             dcd->reverse, dcd->charmm);
      if (rc!=MOLFILE_SUCCESS) {
          close_file_read(dcd);
          return NULL;
      }
      fio_fseek(dcd->fd, dcd->start, FIO_SEEK_SET);
  }
...
static int 
read_timestep_metadata(void *v, molfile_timestep_metadata_t *m) {
    dcdhandle *dcd = (dcdhandle *)v;
    m->has_velocities = 0;
    m->count = dcd->nsets;
    m->supports_double_precision = 0;
    return MOLFILE_SUCCESS;
}

static int read_timestep2(void *v, molfile_ssize_t i, molfile_timestep_t *ts) {
  dcdhandle* dcd = (dcdhandle *)v;
  fio_size_t pos = dcd->start;
  if (i<0) return MOLFILE_ERROR;
  if (i>0) pos += dcd->firstframesize;
  if (i>1) pos += dcd->framesize * (i-1);
  fio_fseek(dcd->fd, pos, FIO_SEEK_SET);
  dcd->setsread = i;
  return read_next_timestep(v,dcd->natoms, ts);
}
...
static int read_next_timestep(void *v, int natoms, molfile_timestep_t *ts) {
  ...
  rc = read_dcdstep(dcd->fd, dcd->natoms, dcd->x, dcd->y, dcd->z, unitcell,
             dcd->nfixed, 0, dcd->freeind, dcd->fixedcoords, 
             dcd->reverse, dcd->charmm);
  if (rc < 0) {  
  ...
  if (unitcell[1] >= -1.0 && unitcell[1] <= 1.0 &&
    ...
    alpha = 90.0 - asin(unitcell[4]) * 90.0 / M_PI_2; /* cosBC */
    beta  = 90.0 - asin(unitcell[3]) * 90.0 / M_PI_2; /* cosAC */
    gamma = 90.0 - asin(unitcell[1]) * 90.0 / M_PI_2; /* cosAB */
  } else {
    ...
    gamma = unitcell[1]; /* angle between A and B */
  }

  molfile_unitcell_from_pdb(ts->unit_cell, a,b,c, alpha, beta, gamma);
...
static int write_timestep(void *v, const molfile_timestep_t *ts) { 
  ...
  molfile_unitcell_to_pdb(ts->unit_cell, &a,&b,&c,&alpha,&beta,&gamma);

  unitcell[0] = a;
  unitcell[2] = b;
  unitcell[5] = c;
  unitcell[1] = sin((M_PI_2 / 90.0) * (90.0 - gamma)); /* cosAB */
  unitcell[3] = sin((M_PI_2 / 90.0) * (90.0 - beta));  /* cosAC */
  unitcell[4] = sin((M_PI_2 / 90.0) * (90.0 - alpha)); /* cosBC */
...
int msys_dcdplugin_register(void *v, vmdplugin_register_cb cb) {
