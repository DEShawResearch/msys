...
const uint32_t desres::Zing::s_two_to_the_27th = 1 << 27;
const uint32_t desres::Zing::s_two_to_the_26th = 1 << 26;
const uint32_t desres::Zing::s_two_to_the_22nd = 1 << 22;

void desres::ZingPool::check_assumptions() {
}

void desres::Zing::check_assumptions() {
  if (sizeof(desres::Zing) != 4) {
    throw dessert("Zing changed size",DESSERT_LOC);
  }

  if (desres::Zing::s_alphabet_5bit.size() != 32) {
    throw dessert("bad alphabet",DESSERT_LOC);/*GCOV-IGNORE*/
  }
  if (desres::Zing::s_alphabet_5bit[31] != 0) {
    throw dessert("bad alphabet doesn't end in NUL",DESSERT_LOC);/*GCOV-IGNORE*/
  }

  if (desres::Zing::s_alphabet_amino.size() != 32) {
    throw dessert("bad alphabet",DESSERT_LOC);/*GCOV-IGNORE*/
  }
  if (desres::Zing::s_alphabet_amino[31] != 0) {
    throw dessert("bad alphabet doesn't end in NUL",DESSERT_LOC);/*GCOV-IGNORE*/
  }

  if (desres::Zing::s_alphabet_4bit.size() != 16) {
    throw dessert("bad alphabet",DESSERT_LOC);/*GCOV-IGNORE*/
  }
  if (desres::Zing::s_alphabet_4bit[15] != 0) {
    throw dessert("bad alphabet doesn't end in NUL",DESSERT_LOC);/*GCOV-IGNORE*/
  }

  if (desres::Zing::s_alphabet_bcd.size() != 16) {
    throw dessert("bad alphabet",DESSERT_LOC);/*GCOV-IGNORE*/
  }
  if (desres::Zing::s_alphabet_bcd[15] != 0) {
    throw dessert("bad alphabet doesn't end in NUL",DESSERT_LOC);/*GCOV-IGNORE*/
  }
}

const size_t desres::ZingPool::s_size = 16;

// Fixed point
// 0ooo oooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_float    = 0x1 << 31;
const uint32_t desres::Zing::control_float = 0x0 << 31;

// 6 chars (5-bit alphabet)
// 10oo oooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_5bit     = 0x3 << 30;
const uint32_t desres::Zing::control_5bit  = 0x2 << 30; 

// 7 chars (BCD 4-bit alphabet)
// 1100 oooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_bcd      = 0xf << 28;
const uint32_t desres::Zing::control_bcd   = 0xc << 28;

// 4 chars (7-bit ASCII)
// 1101 oooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_7bit     = 0xf << 28;
const uint32_t desres::Zing::control_7bit  = 0xd << 28; 

// Pool (1..poolsize)
// 1110 oooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_size     = 0xf << 28;
const uint32_t desres::Zing::control_size  = 0xe << 28;

// Pool (overflow)
// 1111 0ooo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_any      = 0x1f << 27;
const uint32_t desres::Zing::control_any   = 0x1e << 27;

// 6 chars (specialized 4-bit)
// 1111 10oo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_4bit     = 0x3f << 26;
const uint32_t desres::Zing::control_4bit  = 0x3e << 26; 

// 5 chars (specialized 5 bit for amino acids)
// 1111 11oo oooo oooo oooo oooo oooo oooo
const uint32_t desres::Zing::mask_amino    = 0x3f << 26;
const uint32_t desres::Zing::control_amino = 0x3f << 26; 

desres::ZingPool::ZingPool()
...
void desres::ZingPool::clearbits() {
...
void desres::ZingPool::touch(size_t n, size_t length) {
...
void desres::ZingPool::finish_compaction() {
  m_inserts_allowed = true;
}


size_t desres::ZingPool::size() const {
  size_t size = 0;
...
size_t desres::ZingPool::insert(const std::string& string) {
...
std::string desres::ZingPool::retrieve(size_t n,size_t length) const {
  if (length == 0) {
    return "";
  } else if (length <= s_size) {
...
void desres::ZingPool::dump() const {
...
uint32_t desres::Zing::add_control(uint32_t x,uint32_t mask,uint32_t control) {
...
const uint32_t desres::Zing::encode_float(const std::string& string) {
  size_t length = string.size();
...
const std::string desres::Zing::decode_float(uint32_t x) {
#ifdef PEDANTIC
...
const std::string
desres::Zing::s_alphabet_5bit("eariotnslcudpmhgbfywkvxzjq_012\"\0",32);

const std::string
desres::Zing::s_alphabet_amino("MVCINUOGHTYEPARSLDW 01234567890\0",32);

const std::string
desres::Zing::s_alphabet_4bit("0123456789 CONH\0",16);

const uint32_t desres::Zing::encode_nbit(const std::string& string, unsigned nbits, unsigned nchars, uint32_t mask, uint32_t control, const std::string& alphabet) {
  size_t length = string.size();
...
// 00 ..... ..... ..... ..... .....
const uint32_t desres::Zing::encode_5bit(const std::string& string) {
...
const std::string desres::Zing::decode_5bit(uint32_t x) {
#ifdef PEDANTIC
...
const uint32_t desres::Zing::encode_4bit(const std::string& string) {
  return encode_nbit(string,4,6,mask_4bit,control_4bit,s_alphabet_4bit);
}

const std::string desres::Zing::decode_4bit(uint32_t x) {
#ifdef PEDANTIC
...
const uint32_t desres::Zing::encode_amino(const std::string& string) {
  return encode_nbit(string,5,5,mask_amino,control_amino,s_alphabet_amino);
}

const std::string desres::Zing::decode_amino(uint32_t x) {
#ifdef PEDANTIC
...
const uint32_t desres::Zing::encode_any(const std::string& string,ZingPool& pool) {
  size_t length = string.size();
...
const std::string desres::Zing::decode_any(uint32_t x,const ZingPool& pool) {
#ifdef PEDANTIC
...
const uint32_t desres::Zing::encode_size(const std::string& string,ZingPool& pool) {
  size_t length = string.size();
...
const std::string desres::Zing::decode_size(uint32_t x,const ZingPool& pool) {
#ifdef PEDANTIC
...
const std::string desres::Zing::s_alphabet_bcd("0123456789.+-eE\0",16);

// 0100 .... .... .... .... .... .... ....
const uint32_t desres::Zing::encode_bcd(const std::string& string) {
  size_t length = string.size();
...
const std::string desres::Zing::decode_bcd(uint32_t x) {
#ifdef PEDANTIC
...
// 0101 ------- ------- ------- -------
const uint32_t desres::Zing::encode_7bit(const std::string& string) {
  size_t length = string.size();
...
const std::string desres::Zing::decode_7bit(uint32_t x) {
#ifdef PEDANTIC
...
desres::Zing::Zing(const Zing& other)
  : m_bits(other.m_bits)
{
}

desres::Zing& desres::Zing::operator=(const desres::Zing& other)
{
  m_bits = other.m_bits;
  return *this;
}

bool desres::Zing::operator==(const desres::Zing& other) const {
...
desres::Zing::Zing(const std::string& string, desres::ZingPool& pool)
  : m_bits(0)
{
...
std::string desres::Zing::string(const desres::ZingPool& pool) const {
...
uint32_t desres::Zing::integer() const {
  return m_bits;
}

desres::Zing::category_t desres::Zing::category() const {
  uint32_t bits = m_bits;
...
bool desres::Zing::is_empty() const {
  return m_bits == 0;
}

void desres::Zing::touch(desres::ZingPool& pool) const {
  uint32_t bits = m_bits;
