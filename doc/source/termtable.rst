...
`TermTable` instance belongs to a single System, and holds terms that are
all of the same type.  A `TermTable` instance be thought of as containing
a number of `Terms`, each of which describes the interactions of a single
tuple of atoms.

Each `Term` in the TermTable is a handle containing three sorts of
